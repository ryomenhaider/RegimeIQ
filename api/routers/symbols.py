...
async def get_available_symbols(user: CurrentUser = Depends(get_current_user)):
    ...
    symbols = await symbols_service.get_available_symbols()
...
async def check_symbol_viability(symbol: str, user: CurrentUser = Depends(get_current_user)):
    ...
    result = await symbols_service.check_viability(symbol)
...
async def get_user_symbols(username: str, user: CurrentUser = Depends(get_current_user)):
    ...
    symbols = await symbols_service.get_user_symbols(username)
    return success_response({"symbols": symbols})
...
async def add_symbol(
    ...
    try:
        result = await symbols_service.add_symbol(username, symbol, user.plan)
...
async def remove_symbol(
    ...
    try:
        await symbols_service.remove_symbol(username, symbol.upper())
